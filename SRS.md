...
| **DEV-12** | §19.2 | Two endpoints added beyond the table: `API-077 GET /metrics` and `API-078 GET /metrics/vetoes`. Both are unauthenticated, and both are computed from stored rows (`scheduler_runs`, `risk_checks`, `orders`, `policies`, `underwriting_decisions`) rather than from counters held in memory. | §23 requires nine counters and four latency histograms, and §19.2 provides no way to read any of them — a metric nobody can query is a metric that does not exist, and `OPS-008` is explicit that the Kernel's per-rule numbers must be "queryable **and displayed**". Deriving them from the ledger rather than instrumenting call sites means a metric can only be wrong if the book is wrong: an in-process registry would reset on every restart, drift the moment a new code path forgot its `increment()`, and give no way to tell which of the two disagreeing numbers had lied. They are unauthenticated because `UI-003` requires the whole dashboard to be readable with no token, and the veto rate is the number a judge should not need a credential to check. |
...
| **API-077** | GET | `/metrics` | No | `OPS-003` counters and `OPS-004` latencies, derived from the ledger (DEV-12) |
| **API-078** | GET | `/metrics/vetoes?limit=` | No | `OPS-008` per-rule Kernel veto counts (DEV-12) |
